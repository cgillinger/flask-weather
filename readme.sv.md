...
🌐 [English](readme.md) · **Svenska** · [Norsk](readme.nb.md) · [Dansk](readme.da.md) · [Suomi](readme.fi.md) · [Deutsch](readme.de.md) · [Français](readme.fr.md) · [Español](readme.es.md)

**GitHub Repository:** [https://github.com/cgillinger/flask-weather](https://github.com/cgillinger/flask-weather)

**En självhostad väderskärm för Raspberry Pi, Synology NAS eller vilken Linux-burk som helst — med valfri prognosleverantör och förstklassigt stöd för Netatmo-väderstation.** Byggd för väggmonterade surfplattor, kioskskärmar och gamla iPads som får nytt liv.

> 🧑‍🔧 **Det här är ett personligt hobbyprojekt som delas i befintligt skick.** Jag byggde det främst för väderskärmen på min egen vägg. Jag lägger upp det på GitHub ifall någon annan vill ha en snygg, självhostad väderskärm – inte minst med riktigt **Netatmo**-stöd – men jag hinner kanske inte svara på buggrapporter eller granska pull requests. Du är varmt välkommen att forka och göra den till din egen. Se [Support & bidrag](#-support--bidrag).

![Flask Weather Dashboard med Netatmo — uppmätt temperatur och luftkvalitet bredvid prognosen](screenshots/hero-netatmo.png)

*Direkt från min egen väggskärm: **uppmätt** temperatur, luftfuktighet, lufttryck och CO₂ från en Netatmo-station (grönt, ”FAKTISK”) bredvid **prognosen** (blått, ”PROGNOS”), med UV-index och femdygnsöversikt.*

## 💡 Vad är det här?

En liten Flask-app som gör vilken ledig skärm som helst till en elegant, alltid-på väderskärm. En strömsnål enhet (en Raspberry Pi, en Synology-NAS eller vilken Linux-maskin som helst) kör servern; valfri surfplatta, telefon eller dedikerad skärm visar den i en webbläsare. Den fungerar direkt med bara en plats angiven, och växer med dig om du äger en Netatmo-väderstation.

- 🌍 **Välj prognoskälla** — SMHI (Norden), eller **YR/met.no** och **Open-Meteo** för **global täckning**. Ingen API-nyckel krävs, och alla leverantörer normaliseras till samma ikoner och animationer.
- 🏠 **Riktiga mätvärden, inte bara prognos** — med en **Netatmo**-station ser du din *faktiska* temperatur, luftfuktighet, lufttryck och inomhus-**CO₂/luftkvalitet** bredvid prognosen. Det är funktionen de flesta väderskärmar saknar.
- 🎨 **Snyggt värre** — glassmorphism i mörkt tema, en cirkulär LED-klocka, färgkodad temperatur/vind/UV, **sex utbytbara ikonpaket** och animerade **regn- och snöeffekter** som följer den riktiga prognosen.
- 🌐 **Talar ditt språk** — åtta gränssnittsspråk med vindterminologi från respektive lands väderinstitut.
- 📟 **Byggd för kiosk** — trimmad för 1920×1080-väggpaneler och webbappar på hemskärmen i iPad/Android.

Allt ställs in i en enda kommenterad `config.py` — ingen databas, inget byggsteg, inget konto krävs för grundfunktionerna.

## 🖼️ Galleri

**Åtta gränssnittsspråk** (vindtermer, veckodagar, barometerord och kompassbokstäver lokaliseras alla):

![Flask Weather Dashboard på engelska, svenska, tyska och franska](screenshots/gallery-languages.png)

**Sex utbytbara ikonpaket** (samma dashboard, olika väderikoner — sätt `ui.icon_pack` eller låt dem rotera automatiskt):

![Sex väderikonpaket](screenshots/gallery-icon-packs.png)
...
- ✅ **Utomhus-luftkvalitet** (European AQI) från närmaste SMHI-mätstation, med global Open-Meteo/CAMS-fallback — ingen nyckel
...
- ✅ Inomhus-**CO₂/luftkvalitet** — visas bredvid utomhusvärdet (välj inomhus, utomhus eller båda)
...
- **Luftkvalitet** (`air_quality.mode`): inomhus-**CO₂** (Netatmo), **utomhus-AQI** (närmaste SMHI-station → global CAMS-fallback, ingen nyckel), eller **båda** sida vid sida. Utomhusrutan visar avståndet till mätstationen; färgkodas enligt EEA-indexet (God → Extremt dålig). I *båda*-läget tar lövet den sämre av de två nivåerna.
...
Så här hanteras varje källa när den inte är tillgänglig:

| Data | Primär källa | Fallback | Om inget finns |
|------|--------------|----------|----------------|
...
| **Sol-tider** | ipgeolocation API | Inbyggd astronomisk beräkning | — |

### Varför trycktrenden har en uppvärmnings-fallback

Netatmos trend mäts från stationens egen tryckhistorik, vilket är den mest exakta källan så snart den håller ett sammanhängande 3-timmarsfönster. Direkt efter en omstart eller ett längre avbrott är fönstret tomt. I stället för att gissa fyller appen luckan med **prognostendensen** (leverantörens förutspådda tryckförändring de kommande 3 timmarna) – ett äkta värde, klassat på samma trösklar som den uppmätta trenden, tillgängligt redan från första uppdateringen. Appen växlar tillbaka till den uppmätta Netatmo-trenden automatiskt så snart tillräcklig historik samlats. Detta är leverantörsoberoende: SMHI, YR och Open-Meteo levererar alla det prognostryck som används här.
...
'air_quality': {
    # 'indoor'  = endast Netatmo-CO₂ (kräver use_netatmo=True)
    # 'outdoor' = endast utomhus-AQI (närmaste SMHI-station → global CAMS-fallback, ingen nyckel)
...
**En vänlig påminnelse:** det här är ett hobbyprojekt som jag pysslar med på fritiden, främst för min egen väderskärm. Jag delar det för att det kan vara användbart för någon annan som vill ha en snygg, självhostad väderskärm — inte som en supporterad produkt. Så:

- 🙂 **Öppna gärna en issue**, men var medveten om att jag kanske inte hinner titta på den (eller svara) snabbt, om alls.
- 🍴 **Forka på!** Vill du ha ändringar är snabbaste vägen att forka och anpassa den till din uppsättning — koden är MIT-licensierad just för det.
- 🔀 **Pull requests får gärna finnas**, men jag kan inte lova granskning eller merge. Inga hårda känslor åt något håll.
- 📖 **Det mesta är redan nedskrivet:** `reference/config_example.py` har utförliga kommentarer, och den här readmen täcker installation, konfiguration och felsökning på djupet.

Tack för förståelsen — och ha det så trevligt med skärmen! 🌤️

### 📚 Resurser

- **GitHub Issues**: [github.com/cgillinger/flask-weather/issues](https://github.com/cgillinger/flask-weather/issues)
- **Konfiguration**: `reference/config_example.py` har detaljerade kommentarer
- **Utvecklingshistorik**: [CHANGELOG.md](CHANGELOG.md) · [DEVELOPMENT_HISTORY.md](DEVELOPMENT_HISTORY.md)
